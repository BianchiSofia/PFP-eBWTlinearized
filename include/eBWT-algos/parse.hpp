...
public:  
          ...
          bool ilP_flag_ = true)
    {
        ...
        tmp_filename = filename + std::string(".seqstart");
        ...
        sdsl::sd_vector_builder builder(size, sts.size());
        for(auto idx: sts) {
            builder.set(idx);
        }
        ...
        buildBitIl(); // build Inverted List bit vectors
...
void makeEBWT(){
        ...
        for (size_t i=0; i<saP.size(); i++){
            ...
            if(b_d[pos]==1){ 
                size_t rank = rank_b_d(pos+1);
                
                size_t last_char_pos = select_b_d(rank_b_d(pos+1)+1)-1;
        
                if (last_char_pos < p.size() && p[last_char_pos] > 0) {
